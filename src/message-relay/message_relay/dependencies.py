...
class MessageRelayService:
    ...
    async def start(self):
        ...
        try:
            for message in self.kafka_helper.consumer:
                ...
                # Update metrics
                metrics.message_relay_messages_received_total.labels(
            ...
            metrics.message_relay_kafka_errors_total.labels(
    ...
    async def process_message(self, message: dict):
        ...
        try:
            ...
            if not user_id:
                ...
                metrics.message_relay_messages_failed_total.labels(
                    error_type="missing_userid"
                ).inc()
                return
            ...
            metrics.message_relay_messages_processed_total.labels(
                status="success"
            ).inc()
            metrics.message_relay_processing_duration_seconds.labels(
                status="success"
    ...
    async def publish_to_redis(self, user_id: str, message: dict):
        ...
        try:
            ...
            if subscribers == 0:
                metrics.message_relay_zero_subscribers_total.labels(
                    channel=channel
                ).inc()
            ...
            metrics.message_relay_redis_publish_errors_total.labels(
                error_type=type(e).__name__
            ).inc()
            logger.error(
